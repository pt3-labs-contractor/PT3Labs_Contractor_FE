import React from 'react';

import './Team.css';

export default function Team() {
  return (
    <div className="team-page">
      <section className="meet-the-team">
        <h1 className="heading">
          Developer<span className="team"> Team</span>
        </h1>
        <div className="card-wrapper">
          <a href="https://github.com/LeeTann">
            <div className="card">
              <img
                src="images/card-bg.jpg"
                alt="card bg img"
                className="card-img"
              />
              <img
                src="images/lee2.jpg"
                alt="profile1 img"
                className="profile-img"
              />
              <h1>Lee Tann</h1>
              <h6>Full-Stack Developer</h6>
              <ul className="social-media">
                <li>
                  <i className="fab fa-twitter-square" />
                </li>
                <li>
                  <i className="fab fa-github" />
                </li>
                <li>
                  <i className="fab fa-linkedin" />
                </li>
              </ul>
            </div>
          </a>

          <a href="https://github.com/badCompany55">
            <div className="card">
              <img
                src="images/card-bg.jpg"
                alt="card bg img"
                className="card-img"
              />
              <img
                src="images/profile1.png"
                alt="profile1 img"
                className="profile-img"
              />
              <h1>Zack Irvin</h1>
              <h6>Full-Stack Developer</h6>
              <ul className="social-media">
                <li>
                  <i className="fab fa-twitter-square" />
                </li>
                <li>
                  <i className="fab fa-github" />
                </li>
                <li>
                  <i className="fab fa-linkedin" />
                </li>
              </ul>
            </div>
          </a>

          <a href="https://github.com/edreeseg">
            <div className="card">
              <img
                src="images/card-bg.jpg"
                alt="card bg img"
                className="card-img"
              />
              <img
                src="images/ed1.jpg"
                alt="profile1 img"
                className="profile-img"
              />
              <h1>Edward Reeseg</h1>
              <h6>Full-Stack Developer</h6>
              <ul className="social-media">
                <li>
                  <i className="fab fa-twitter-square" />
                </li>
                <li>
                  <i className="fab fa-github" />
                </li>
                <li>
                  <i className="fab fa-linkedin" />
                </li>
              </ul>
            </div>
          </a>

          <a href="https://github.com/ExpressoJava">
            <div className="card">
              <img
                src="images/card-bg.jpg"
                alt="card bg img"
                className="card-img"
              />
              <img
                src="images/profile1.png"
                alt="profile1 img"
                className="profile-img"
              />
              <h1>Bao Pham</h1>
              <h6>Full-Stack Developer</h6>
              <ul className="social-media">
                <li>
                  <i className="fab fa-twitter-square" />
                </li>
                <li>
                  <i className="fab fa-github" />
                </li>
                <li>
                  <i className="fab fa-linkedin" />
                </li>
              </ul>
            </div>
          </a>

          <a href="https://github.com/echopecho">
            <div className="card">
              <img
                src="images/card-bg.jpg"
                alt="card bg img"
                className="card-img"
              />
              <img
                src="images/phil-smith.jpg"
                alt="profile1 img"
                className="profile-img"
              />
              <h1>Phillip Smith</h1>
              <h6>Full-Stack Developer</h6>
              <ul className="social-media">
                <li>
                  <i className="fab fa-twitter-square" />
                </li>
                <li>
                  <i className="fab fa-github" />
                </li>
                <li>
                  <i className="fab fa-linkedin" />
                </li>
              </ul>
            </div>
          </a>

          <a href="https://github.com/dlimla">
            <div className="card">
              <img
                src="images/card-bg.jpg"
                alt="card bg img"
                className="card-img"
              />
              <img
                src="images/daniel1.jpg"
                alt="profile1 img"
                className="profile-img"
              />
              <h1>Daniel Lim</h1>
              <h6>Full-Stack Developer</h6>
              <ul className="social-media">
                <li>
                  <i className="fab fa-twitter-square" />
                </li>
                <li>
                  <i className="fab fa-github" />
                </li>
                <li>
                  <i className="fab fa-linkedin" />
                </li>
              </ul>
            </div>
          </a>

          <a href="https://github.com/henron1">
            <div className="card">
              <img
                src="images/card-bg.jpg"
                alt="card bg img"
                className="card-img"
              />
              <img
                src="images/profile1.png"
                alt="profile1 img"
                className="profile-img"
              />
              <h1>Henry Neal</h1>
              <h6>PM</h6>
              <ul className="social-media">
                <li>
                  <i className="fab fa-twitter-square" />
                </li>
                <li>
                  <i className="fab fa-github" />
                </li>
                <li>
                  <i className="fab fa-linkedin" />
                </li>
              </ul>
            </div>
          </a>
        </div>
      </section>
    </div>
  );
}
